...
**개발자가 Canton 을 이해하고, Daml 로 컨트랙트를 개발할 수 있도록 한다.**

Step 을 순서대로 거치면서 그 목표에 도달한다. 각 Step 은 **문서**와 **러너
스크립트** 한 쌍이다. 러너는 하나의 터미널에서 설명 → 엔터 → 실행 → 결과 순으로
돈다.
...
Bob 에게 이체하고, Bob 이 Goldman Sachs 의 채권과 교환한다.** 등장 인물은 Step 01 에
정의되어 있다.
...
러너를 먼저 돌리고 해당 Step 문서를 읽는 순서를 권한다. 문서에는 러너가 보여준 것의
의미와 남겨야 할 결론이 정리되어 있다.
...
- **`daml` 어시스턴트는 쓰지 않는다.** 3.5 에서 제거되었고 [DPM](https://docs.canton.network/sdks-tools/cli-tools/dpm) 이 대체한다.
...
패키지 매니저로 설치한 JDK 는 거기 등록되지 않는 경우가 있다. `JAVA_HOME` 을 직접
지정하면 된다.
...
DPM 캐시에서 자동으로 찾지만 못 찾으면 직접 지정한다.
