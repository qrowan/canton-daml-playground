...
[Step 02](Step02Environment.md) 는 원장을 관찰했습니다. 여기서는 **코드를 씁니다.**

Daml 문법과 권한 계산 규칙을 익히고, `dpm test` 로 검증하는 개발 루프를 만듭니다.
그리고 예금 이체가 왜 아직 불가능한지 확인합니다.
...
Step 02 의 `daml/Step02/Deposit.daml` 은 그대로 있습니다. Step 02 러너를 다시 실행해도
동작합니다.
...
필드를 건드리지 않았으므로 Step 02 러너는 그대로 동작합니다.
...
`signatory` 는 동의가 필요한 party, `ensure` 는 원장 기록 전 항상 검사되는 불변식입니다.

**template 에 선언되지 않은 것은 존재하지 않습니다.** `onlyOwner` 로 막는 게 아니라
애초에 그런 행위가 없습니다.
...
기본은 **consuming** 입니다. 행사되면 그 계약은 archive 됩니다. 소비하지 않으려면
`nonconsuming` 을 앞에 붙입니다.

`controller` 는 `signatory` 와 별개다. signatory 가 아닌 party 도 controller 가 될
수 있고, Step 04 에서 그 형태가 나옵니다.
...
**설계 함의**: template 에 choice 를 하나 추가하는 것은 조항을 하나 더 넣는 것입니다.
가볍게 볼 일이 아닙니다.
...
`testPartialWithdraw` 가 옛 계약 ID 로 조회하면 `None` 임을 확인합니다.

**실무 함의**: 계약 ID 를 외부 시스템에 저장해두면 곧 무효가 됩니다. 조회는 `query` 로
다시 하는 것이 원칙입니다.
...
컴파일은 되고 실행에서 실패합니다. `testTransferFails` 가 이것을 문서화합니다.

**버그가 아닙니다.** Deposit 은 owner 의 서명이 필요한 계약인데, 아직 아무 관계도 없는
Bob 이 서명했을 리 없습니다. 남에게 원치 않는 채권·채무를 떠넘길 수 없다는 뜻이고 Daml 이
의도한 안전장치다.

현실에서도 내 예금 계약의 명의를 상대 동의 없이 남에게 넘길 수는 없습니다.

해결은 Step 04 의 propose/accept 다.

### 4. dpm test 는 Canton 이 아닙니다

인메모리 엔진으로 스크립트를 실행합니다. Sequencer·Mediator·확인 프로토콜이 없습니다.
...
성공한 적이 없기 때문입니다. `Archive` 는 이 테스트에서 직접 부르지 않았다.
