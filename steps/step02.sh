...
#
# 하나의 터미널에서 설명 → 엔터 → 실행 → 결과 순으로 진행합니다.
...
say "daml.yaml 의 init-script 를 실행하지 않으므로 party 가 하나도 없습니다."
say ""
...
note "participant 가 synchronizer 에 연결되어야 비로소 party 를 만들 수 있습니다."
...
title "원장이 비어 있는지 확인"
say "party 목록을 조회합니다. sandbox::... 하나만 있어야 합니다."
say "그것은 participant 자신의 admin party 이고 업무용 party 가 아닙니다."
pause
...
note "participant_admin 은 ParticipantAdmin 권한을 가진 노드 운영 계정입니다."
note "어떤 party 도 대리하지 않습니다. 다음 두 단계는 이 계정 권한으로 하는 운영 작업입니다."
...
say "party 만으로는 커맨드를 제출할 수 없습니다. 인증이 꺼져 있어도 user 가 필요하다."
say ""
...
say "트랜잭션은 template 을 package-id 로 지목합니다. 코드가 1바이트만 바뀌어도"
say "해시가 달라지므로 누가 몰래 다른 로직으로 바꿔치기할 수 없습니다."
pause

export PKG=$(dpm inspect-dar "$DAR" 2>/dev/null | grep -oE "[0-9a-f]{64}" | head -1)
ok "PKG = $PKG"
printf '\n'
say "원장에 업로드·vetting 된 것과 같은지 JSON API 로 대조합니다."
run "curl -s \$API/v2/packages | grep -c $PKG"
note "일치합니다. vetting 은 participant 가 '이 package-id 를 쓰겠다'고 공표하는 것이고,"
note "--dar 로 넘겼으므로 sandbox 가 업로드와 vetting 을 함께 처리했습니다."
...
printf '\n'
say "${B}David 에게는 아무것도 보이지 않습니다.${R} David 는 이 계약의 signatory 도 observer 도"
say "아니므로 stakeholder 가 아니고, 데이터가 전달되지 않습니다."
printf '\n'
warn "단 지금은 participant 가 하나다. 물리적으로는 같은 노드가 양쪽 데이터를 갖고 있고"
warn "Ledger API 가 party 단위로 뷰를 분리해 보여주는 것입니다. participant 가 서로 다를 때"
warn "비로소 데이터 자체가 도달하지 않습니다. Step 05 에서 확인합니다."
...
printf '\n'
say "${B}세 가지를 확인할 것.${R}"
say "  1. ${B}david-web 이라는 user 이름이 에러에 없습니다.${R} user 는 'David 를 주장해도 되는가'만"
say "     판정하고 사라졌다. Daml 엔진에는 party 만 도달합니다."
say "  2. ${B}requires authorizers${R} — createArguments 에 owner 를 Alice 로 쓴 것이"
say "     Alice 의 권한을 만들어주지 않았다. 권한은 template 의 signatory 선언에서 나옵니다."
say "  3. 앞의 64자 해시 — 어떤 코드로 검증했는지가 에러에 기록됩니다."
...
cat <<SUMMARY
  ...
  ${B}다음${R}  Step 03 — Daml 문법과 권한 계산 규칙. choice 를 추가하고
        이체가 왜 아직 불가능한지 확인합니다.
...
if [ "$KEEP" = 1 ]; then
  ...
  say "sandbox 를 종료합니다. 인메모리이므로 party·user·계약이 모두 사라집니다."
  note "다시 실행하면 namespace 지문부터 새로 생성됩니다."
