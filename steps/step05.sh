...
cat <<'BANNER'
 ...
 여기서는 노드를 직접 띄웁니다.
   ...
   bank-participant     Bank, Alice 호스팅   Ledger 5011 / JSON 5013
   broker-participant   Bob 호스팅           Ledger 5021 / JSON 5023
   public-sequencer     순서 부여             5001
   public-mediator      판정                  5003
...
export BANK ALICE BOB

ok "노드 4개 기동, Synchronizer 'public' 구성 완료"
...
printf '  %-8s %s\n' "Bob"   "${BOB##*::}"
printf '\n'
if [ "${BANK##*::}" = "${ALICE##*::}" ] && [ "${BANK##*::}" != "${BOB##*::}" ]; then
  ok "Bank 와 Alice 는 같고, Bob 은 다릅니다"
else
  warn "예상과 다릅니다"
fi
printf '\n'
say "Bank 와 Alice 는 ${B}bank-participant${R} 가 발급했고, Bob 은"
say "${B}broker-participant${R} 가 발급했습니다. Party ID 의 뒷부분이"
...
printf '%s$ GET %s/v2/parties   (bank-participant)%s\n\n' "$YE" "$BANK_API" "$R"
listparties "$BANK_API"

printf '\n%s$ GET %s/v2/parties   (broker-participant)%s\n\n' "$YE" "$BROKER_API" "$R"
listparties "$BROKER_API"
...
say "Bootstrap 이 bank 와 broker 양쪽에 업로드했습니다."
pause
...
for pair in "bank:$BANK_API" "broker:$BROKER_API"; do
  name="${pair%%:*}"; api="${pair#*:}"
...
title "발행 — Bank 와 Alice 는 같은 노드라 한 번에 된다"
say "bank-participant 가 두 Party 를 모두 호스팅하므로 actAs 에 둘 다 넣을 수"
...
CREATE="[{\"CreateCommand\":{\"templateId\":\"$DEP\",\"createArguments\":{\"bank\":\"$BANK\",\"owner\":\"$ALICE\",\"amount\":\"100.0\"}}}]"
submit_cmd "$BANK_API" bank-settlement "[\"$BANK\",\"$ALICE\"]" "$CREATE" \
  | result
...
title "Broker 노드에는 이 Contract 가 존재하지 않는다"
...
printf '%s$ bank-participant 에서 Alice 시점 조회%s\n\n' "$YE" "$R"
acs "$BANK_API" "$ALICE" | jq_ 'print("  활성 Contract:", len(d), "건")'

printf '\n%s$ broker-participant 에서 Bob 시점 조회%s\n\n' "$YE" "$R"
acs "$BROKER_API" "$BOB" | jq_ 'print("  활성 Contract:", len(d), "건")'

printf '\n%s$ broker-participant 에서 Alice 시점 조회 시도%s\n\n' "$YE" "$R"
acs "$BROKER_API" "$ALICE" | jq_ 'print("  결과:", d)'
...
printf '%s$ broker 노드에서 Alice 권한으로 제출 시도%s\n\n' "$YE" "$R"
submit_probe="[{\"CreateCommand\":{\"templateId\":\"$PKG:Step04.Deposit:Deposit\",\"createArguments\":{\"bank\":\"$BANK\",\"owner\":\"$ALICE\",\"amount\":\"1.0\"}}}]"
curl -s -X POST "$BROKER_API/v2/commands/submit-and-wait" -H 'Content-Type: application/json' \
  -d "{\"commands\":$submit_probe,\"commandId\":\"s05-probe\",\"userId\":\"bob-web\",\"actAs\":[\"$ALICE\"],\"readAs\":[]}" \
  | result

printf '\n'
warn "broker 노드는 Alice 를 호스팅하지 않으므로 Alice 로 제출할 수 없습니다."
...
say "Alice 는 bank 노드, Bob 은 broker 노드에 있습니다."
say "어느 노드도 두 Party 의 권한을 동시에 갖지 못합니다."
pause

say "먼저 편법을 시도해 봅니다 — bank 노드에서 actAs 에 Bob 을 넣습니다."
printf '\n'
BADCREATE="[{\"CreateCommand\":{\"templateId\":\"$DEP\",\"createArguments\":{\"bank\":\"$BANK\",\"owner\":\"$BOB\",\"amount\":\"50.0\"}}}]"
submit_cmd "$BANK_API" bank-settlement "[\"$BANK\",\"$BOB\"]" "$BADCREATE" \
  | result

printf '\n'
ok "거부되었습니다. bank 노드는 Bob 의 권한을 행사할 수 없습니다"
printf '\n'
say "${B}Step 04 에서 만든 propose/accept 로 갑니다.${R}"
pause

ALICE_DEP=$(wait_contract "$BANK_API" "$ALICE" Deposit) || die "Alice 의 예금을 찾지 못했습니다"

printf '%s$ TX 1 — bank 노드에서 Alice 가 ProposeTransfer%s\n\n' "$YE" "$R"
PROP="[{\"ExerciseCommand\":{\"templateId\":\"$DEP\",\"contractId\":\"$ALICE_DEP\",\"choice\":\"ProposeTransfer\",\"choiceArgument\":{\"newOwner\":\"$BOB\"}}}]"
submit_cmd "$BANK_API" alice-web "[\"$ALICE\"]" "$PROP" \
  | result

printf '\n'
say "제안이 만들어졌습니다. Bob 이 Observer 이므로 ${B}broker 노드에도 도달${R}합니다."
printf '\n'
printf '%s$ broker 노드에서 Bob 시점 조회%s\n\n' "$YE" "$R"
PROP_CID=$(wait_contract "$BROKER_API" "$BOB" TransferProposal) \
  || die "제안이 broker 노드에 도달하지 않았습니다"
acs "$BROKER_API" "$BOB" | jq_ '
...
submit_cmd "$BROKER_API" bob-web "[\"$BOB\"]" "$ACC" \
  | result

printf '\n'
say "결과를 양쪽에서 봅니다."
printf '\n'
printf '  %s bank-participant / Alice%s\n' "$B" "$R"
acs "$BANK_API" "$ALICE" | jq_ 'print("    활성 Contract:", len(d), "건")'
printf '  %s broker-participant / Bob%s\n' "$B" "$R"
wait_contract "$BROKER_API" "$BOB" Deposit >/dev/null || true
acs "$BROKER_API" "$BOB" | jq_ '
print("    활성 Contract:", len(d), "건")
for e in d:
    c=e["contractEntry"]["JsActiveContract"]["createdEvent"]
