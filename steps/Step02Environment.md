...
[Step 01](Step01Terminology.md) 에서 정의한 용어가 실제로 무엇인지 눈으로 확인한다.
...
하나의 터미널에서 **설명 → 엔터 → 실행 → 결과** 순으로 12단계를 진행한다.
Sandbox 기동과 종료도 스크립트가 처리한다.
...
`signatory` 가 둘이므로 **은행과 예금주 양쪽의 권한**이 있어야 계약이 성립한다.
...
| 3 | Sandbox 기동 | Participant + Sequencer + Mediator 가 한 JVM 에. **synchronizer 연결까지 기다려야 한다** |
...
3단계에서 `/v2/state/connected-synchronizers` 를 확인한다. Participant 가
synchronizer 에 연결되기 전에 party 를 만들려 하면 이렇게 거부된다.
...
이 지문이 **이 party 들을 발급·인가한 키**의 것이다. `partyIdHint` 로 준 이름은
힌트일 뿐이고 식별자는 전체 문자열이다.

### 3. Party 만으로는 아무것도 못 한다
...
6단계에서 `citi-settlement` 에 Citi 와 Alice 두 party 의 `CanActAs` 를 준다.
은행 백오피스가 자기 명의와 고객 명의를 모두 대리하는 실제 구성이다.

### 4. 권한은 필드가 아니라 signatory 선언에서 나온다

8단계와 11단계가 같은 것을 두 각도에서 보여준다.
...
Participant 가 서로 다를 때 비로소 **데이터 자체가 도달하지 않는다.** Step 05 에서
노드를 분리해 확인한다.

## Sandbox 는 인메모리다

종료하면 party·user·계약뿐 아니라 **participant 의 키까지 사라진다.** 재실행하면
namespace 지문부터 새로 생성되므로, 이전 실행의 party ID 는 무효다.

그래서 `step02.sh` 는 party ID 와 package-id 를 하드코딩하지 않고 매번 조회한다.
...
Participant 가 **1개뿐**이라 Canton 고유의 것 대부분이 재현되지 않는다.
